...
MapWidget::MapWidget()
{
    //读取地图信息--用于读取描述地图信息的文件(包括地图名及经纬度等信息) 用武汉地图，有点失真
    readMap ();
    zoom = 50;
    int width = map.width ();
    ...
    zoominLabel->setPixmap (QPixmap(":/resource/zoomin.png"));
    QLabel *zoomoutLabel = new QLabel;
    zoomoutLabel->setScaledContents (true);
    zoomoutLabel->setPixmap (QPixmap(":/resource/zoomout.png"));
...
void MapWidget::readMap ()
{
    ...
    QFile mapFile(":/resource/maps.txt");
    //以"只读"的方式打开此文件
    ...
    map.load (":/resource/" + mapName);      //将地图读取至私有标量map中
}
